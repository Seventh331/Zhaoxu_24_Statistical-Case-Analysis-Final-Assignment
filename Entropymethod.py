...
class EntropyMethod:
            ...
            for column in uniform_mat.columns:  # 对于此行每一列
                if column in self.negative:  # 如果这一列属于负向指标
                    uniform_mat[column][i] = (uniform_mat[column][i] - min_index[column]) / (
                                max_index[column] - min_index[column])  # 分别归一化
                else:
    ...
    def calc_probability(self):
        ...
        for column in p_mat.columns:
            ...
            p_mat[column] = p_mat[column].apply(
                lambda x_i_j: x_i_j / sigma_x_1_n_j if x_i_j / sigma_x_1_n_j != 0 else 1e-6)  # 计算每个实例占这个指标的比重
    ...
    def calc_entropy(self):
        ...
        e_j = -(1 / np.log(len(self.p_mat))) * np.array(
            [sum([pij * np.log(pij) for pij in self.p_mat[column]]) for column in self.p_mat.columns])  # 计算指标的熵值
